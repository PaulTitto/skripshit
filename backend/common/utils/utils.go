...
func BindFromJson(dest any, filename, path string) error {
	...
	if err != nil {
		logrus.Error("failed to unmarshal json %v", err)
		return err
	}
	return nil
}
...
func SetEnvConsulKV(v *viper.Viper) error {
	...
	err := v.Unmarshal(&env)
	if err != nil {
		logrus.Error("failed to unmarshal env %v", err)
		return err
	}
	...
	for k, v := range env {
		...
		default:
			panic("Unsupported env var")
		}

		err = os.Setenv(k, val)
		if err != nil {
			logrus.Error("failed to set env var: %v", err)
			return err
		}
	}
	return nil
...
func BindFromConsul(dest any, endPoint, path string) error {
	...
	if err != nil {
		logrus.Error("Failed to add remote config: %v", err)
		return err
	}
	...
	if err != nil {
		logrus.Error("Failed to set env from consul kv: %v", err)
		return err
	}
